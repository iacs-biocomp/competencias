...
export class CatCompetencialesService {
	...
	async delete(catComp: ICatComp): Promise<boolean> {
		await this.httpClient
			.delete(`${cnf.apiURL}/catCompetenciales/${catComp.id}`)
			.toPromise();
		return true;
	}
	 ...
	 */
		 public getAllCatComp(): Promise<ICatComp[]> {
			return this.httpClient
				.get<ICatComp[]>(`${cnf.apiURL}/catCompetenciales/all`)
				.toPromise();
		}
	...
	async borrarCatComp(id: string): Promise<boolean> {
		...
		try {
			borrado = await this.httpClient
				.delete<boolean>(`${cnf.apiURL}/catCompetenciales/${id}`)
				.toPromise();
		} catch (error) {
			console.log(error);
			alert(
				'No se ha podido borrar esa categoría competencial, contacte con un administrador.'
			);
		}
		return borrado;
	}
	...
	addCatComp(catComp: ICatComp): Promise<boolean> {
		return this.httpClient
			.post<boolean>(`${cnf.apiURL}/catCompetenciales`, catComp)
			.toPromise();
	}
}
